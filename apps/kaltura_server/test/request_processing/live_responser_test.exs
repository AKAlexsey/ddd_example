defmodule KalturaServer.RequestProcessing.LiveResponserTest do
  ...
  describe "#make_response" do
    setup do
      ...
      mpd_pr_redirect_path_without_port = "http://#{domain_name1}/mpd_pr/#{mpd_pr_stream_path}"

      mpd_pr_redirect_path_with_port =
        "http://#{domain_name2}:#{port}/mpd_pr/#{mpd_pr_stream_path}"

      mpd_common_redirect_path_without_port =
        "http://#{domain_name1}/mpd_pr/#{mpd_common_stream_path}"

      mpd_common_redirect_path_with_port =
        "http://#{domain_name2}:#{port}/mpd_pr/#{mpd_common_stream_path}"

      mpd_wv_redirect_path_without_port = "http://#{domain_name1}/mpd_wv/#{mpd_wv_stream_path}"

      mpd_wv_redirect_path_with_port =
        "http://#{domain_name2}:#{port}/mpd_wv/#{mpd_wv_stream_path}"

      mpd_none_redirect_path_without_port = "http://#{domain_name1}/mpd/#{mpd_none_stream_path}"

      mpd_none_redirect_path_with_port =
        "http://#{domain_name2}:#{port}/mpd/#{mpd_none_stream_path}"

      hls_redirect_path_without_port = "http://#{domain_name1}/hls/#{hls_stream_path}"

      hls_redirect_path_with_port = "http://#{domain_name2}:#{port}/hls/#{hls_stream_path}"
    ...
    test "Redirect to right path if appropriate server exist #6 stream_meta hls", %{
      hls_conn: conn,
    ...
    test "Return error if there is no TvStream with given codec and protocol", %{
      ...
      assert assert {^conn, 500, "Server not found"} = LiveResponser.make_response(conn)
    end
  end
