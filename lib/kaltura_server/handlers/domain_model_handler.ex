defmodule CtiKaltura.Handlers.DomainModelHandler do
  ...
  def handle(event, %{model_name: name, attrs: attrs}) do
    raise "CtiKaltura.Handlers.DomainModelHandler unknown model name #{inspect(name)} event: #{
            event
          } attrs #{inspect(attrs)}"
